...
const CompanyDetails: React.FC<Props> = ({ user, onLogout }) => {
  ...
  const [deleteTargetUser, setDeleteTargetUser] =
    useState<CompanyUserRow | null>(null);

  const [editOpen, setEditOpen] = useState(false);

  // ✅ modal edycji "mnie" (chip)
  const [editMeOpen, setEditMeOpen] = useState(false);

  // ✅ modal edycji użytkownika z tabeli
  const [editUserOpen, setEditUserOpen] = useState(false);
  const [editTargetUser, setEditTargetUser] = useState<CompanyUserRow | null>(
    null
  );

  // (opcjonalnie) blokada przy zmianie roli / usuwaniu / edycji
  const [busyUserId, setBusyUserId] = useState<number | null>(null);
  ...
  const handleEditMeSubmit = async (p: EditUserPayload) => {
    await updateAdminUserFull(user.id, {
      email: p.email,
      // canEditRole jest zabezpieczone w UI, ale i tak bezpiecznie:
  ...
  // ✅ edycja usera z tabeli (SYSTEM_ADMIN może edytować wszystkich)
  const handleEditAnyUserSubmit = async (
    userId: number,
    p: EditUserPayload
  ) => {
    setError(null);
  ...
  return (
    <div className="cd-page">
      <div className="cd-shell">
          ...
          <div className="cd-left">
            ...
            <div>
              ...
              <div className="cd-subtitle">
                Zarządzanie firmą i użytkownikami
              </div>
            </div>
          </div>
              ...
              <div className="cd-card">
                ...
                <div className="cd-kv">
                  ...
                  <div className="cd-kvrow">
                    <span>Status VAT</span>
                    <strong>
                      {company.vatActive ? "czynna" : "nieczynna"}
                    </strong>
                  </div>
                </div>
              </div>
                          ...
                          <tr key={u.id}>
                            ...
                            <td>
                              ...
                              >
                                <option value="ROLE_EMPLOYEE">Pracownik</option>
                                <option value="ROLE_MANAGER">Kierownik</option>
                                <option value="ROLE_COMPANY_ADMIN">
                                  Admin firmy
                                </option>
        ...
        {/* ✅ modal edycji aktualnie zalogowanego usera (chip) */}
        {editMeOpen && (
          <UserEditModal
            title="Edytuj swoje dane"
            initial={{
        ...
        {/* ✅ modal edycji wybranego usera z tabeli */}
        {editUserOpen && editTargetUser && (
          <UserEditModal
            ...
            canEditRole={true} // SYSTEM_ADMIN edytuje role innych
            onClose={() => {
              setEditUserOpen(false);
              setEditTargetUser(null);
            }}
